...
namespace Authentication.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, DrugStore_AuthenticationContext context)
        {
            _logger = logger;
            _context = context;
        }
        ...
        private IQueryable<Product> Filter(IQueryable<Product> products, int? categoryId, string? description, double? fromPrice, double? toPrice, string? op)
        {
            ...
            //products = products.OrderBy("Available DESC");
            return products;
        }
        private IQueryable<Product> Sort(IQueryable<Product> products, string? orderBy, string? orderType, string? op)
        {
            ...
            products = products.OrderBy($"{sortInfo.orderBy} {sortInfo.orderType}");
            //products = products.OrderBy("Available DESC");
            return products;
        }
    }
